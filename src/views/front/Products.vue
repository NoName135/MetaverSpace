            ...
            >
              ...
              >
                ...
                >
                  ...
                  <h3 class="mt-2 text-xl font-bold">{{ product.title }}</h3>
...
export default {
  ...
  methods: {
    ...
    getFilterProducts(page = 1) {
      ...
      this.products = this.filterProducts.filter((item, i) => {
        return Math.ceil((i + 1) / 12) == page;
      });
      this.productImages = [];
    ...
    // 切換頁碼
    changePages(page = 1) {
      this.products = Object.values(this.filterProducts).filter((item, i) => {
        return Math.ceil((i + 1) / 12) == page;
      });
